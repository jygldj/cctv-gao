let _data={
    ...
    channels(){
        _data.vue.channels.push({id:"tv",tag:"tv",name:"电视剧",loading:false,pageNum:0,vods:[],noMore:false});
        _data.vue.channels.push({id:"dhp",tag:"dhp",name:"动画片",loading:false,pageNum:0,vods:[],noMore:false});
        _data.vue.channels.push({id:"jlp",tag:"jlp",name:"纪录片",loading:false,pageNum:0,vods:[],noMore:false});
        _data.vue.channels.push({id:"tbjm",tag:"tbjm",name:"特别节目",loading:false,pageNum:0,vods:[],noMore:false});
    },
    channelPage(channelItem){
        ...
        let requestUrl="https://api.cntv.cn/list/getVideoAlbumList?serviceId=tvcctv&n=200"+
    ...
    /**
     * 用专辑接口直接解析出「第一集播放页」的 URL。
     *
     * 原链路：片库点节目 → 加载央视专辑页 → cctvFullscreen 自动点「立即观看」→ 播放页。
     * 中间那次专辑页是一次完整的央网页加载，纯属白等。
     * 这里提前把播放页 URL 拿出来直跳，省掉这一整段；任一步失败都回调 null，
     * 调用方回退到原专辑页逻辑，不影响可用性。
     */
    playUrlOf(albumId, callback){
        if(!albumId){ callback(null); return; }
        try{
            let api="https://api.cntv.cn/NewVideo/getVideoListByAlbumIdNew?id="+encodeURIComponent(albumId)+
                "&serviceId=tvcctv&pub=1&part=0&n=1&sort=asc";
            _apiX.getJson(api,
                { "User-Agent": _apiX.userAgent(false), "tv-ref": "https://tv.cctv.com/" },
                function(text){
                    try{
                        ...
                        if(list.length>0&&list[0].url){ callback(list[0].url); return; }
                    }catch(e){}
                    callback(null);
                },
